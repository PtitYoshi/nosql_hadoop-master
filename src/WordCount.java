...
public class WordCount {
	...
	private static final int OCCUR_MIN = 3;
...
//			LOG.info("MESSAGE INFO");
			...
			//Les mots sont filtrés par rapport aux espaces et aux symboles de ponctuations. On prends en compte la possibilité que plusieurs de ces éléments se suivent.
			...
			//Pour chacun des mots, on ecrit les mots dans la structure avec une itération d'occurence (si déjà écrit, on incrémente l'occurence).
			for (String word : words)
			{
				if(word.length()>LENGTH_MIN)				//Filtre par rapport à la taille des mots.
	...
	public static class Reduce extends Reducer<Text, IntWritable, Text, IntWritable> {
		...
		public void reduce(Text key, Iterable<IntWritable> values, Context context)
			...
			//On calcule les occurences d'un même mot enregistré dans le Mapper.
			for (IntWritable val : values)
				sum += val.get();
			
			//On filtre par rapport aux nombre d'occurence des mots.
			if(sum>OCCUR_MIN)
